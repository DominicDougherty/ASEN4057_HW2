...
while (contloop==1) %infinite loop to continue while data in the files
    ...
    dmin=min([di2 dq2]);
    d3min=min([d3]);
    plot(app.UIAxes4,spacing2,di2,'r.');hold on
    plot(app.UIAxes4,spacing2,di2,'c');hold off
    axis([(spacing2(1)-0.1) (spacing2(end)+0.1) (dmin-0.05) 1.05])
    title(app.UIAxes4,'direct (I)')
    plot(app.UIAxes4_3,spacing2,dq2,'g.');hold on
    plot(app.UIAxes4_3,spacing2,dq2,'c');hold off
    axis([(spacing2(1)-0.1) (spacing2(end)+0.1) (dmin-0.05) 1.05])
    title(app.UIAxes4_3,'direct (Q)')
    plot(app.UIAxes5,spacing2,d3,'b.');hold on
    plot(app.UIAxes5,spacing2,d3,'m');hold off
    axis([(spacing2(1)-0.1) (spacing2(end)+0.1) (d3min-0.05) 1.05])
    powerrat(loopcnt) = rrmaxrat / ddmax;
    
    %reflected signal
    title(app.UIAxes5,['reflected/direct ratio: ',num2str(powerrat(loopcnt))])
    rmin=min([ri2 rq2]);
    r3min=min([r3]);
    plot(app.UIAxes4_2,-spacing(end:-1:1),ri2,'r.');hold on
    plot(app.UIAxes4_2,-spacing(end:-1:1),ri2,'c');hold off
    axis([(-spacing(end)-0.1) (-spacing(1)+0.1) (rmin-0.05) 1.05])
    title(app.UIAxes4_2,'reflect (I)')
    plot(app.UIAxes4_4,-spacing(end:-1:1),rq2,'g.');hold on
    plot(app.UIAxes4_4,-spacing(end:-1:1),rq2,'c');hold off
    axis([(-spacing(end)-0.1) (-spacing(1)+0.1)  (rmin-0.05) 1.05])
    title(app.UIAxes4_4,'reflect (Q)')
    plot(app.UIAxes6,-spacing(end:-1:1),r3,'b.');hold on
    plot(app.UIAxes6,-spacing(end:-1:1),r3,'m');hold off
    ...
    title(app.UIAxes6,['reflected composite; path delay of ',num2str(pathdelay(loopcnt),6),'m '])
    xlabel(app.UIAxes6,['time elapsed : ',num2str(loopcnt*avgnum/(60*1000),4),' min; Int#:',int2str(loopcnt)])
